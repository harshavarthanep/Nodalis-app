...
export async function push(dirHandle, onProgress = () => {}) {
  ...
  for (const note of notes) {
    await pushOne(dirHandle, note);
    done++; onProgress(done, notes.length);
  }
  return { pushed: notes.length };
}

/** Write a single note to disk — used for continuous auto-backup so every
 * keystroke doesn't have to rewrite the whole vault. */
export async function pushOne(dirHandle, note) {
