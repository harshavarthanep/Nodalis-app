 ...
 */
import { state, bus, noteTitle, createNote, togglePin, duplicateNote } from './state.js';
import { setActiveView, showToast } from './layout-manager.js';
...
function getCommands() {
  const cmds = [
    ...
    { label: 'Open help & feature manual', hint: '?', action: () => bus.emit('help:open') },
    { label: 'Replay the guided tour', hint: '', action: () => bus.emit('tour:start') },
    { label: 'Toggle theme', hint: '', action: () => bus.emit('theme:toggle') },
    { label: 'Open customization panel', hint: '', action: () => { setActiveView('settings'); bus.emit('settings:scrollTo', 'customization'); } },
    { label: 'Export vault as .zip', hint: '', action: () => bus.emit('sync:export') },
    { label: 'Sync now', hint: '', action: () => bus.emit('sync:now') },
  ];
  if (state.activeNoteId && state.notes.has(state.activeNoteId)) {
    const note = state.notes.get(state.activeNoteId);
    cmds.push(
      { label: note.pinned ? 'Unpin current note' : 'Pin current note', hint: '', action: async () => { const p = await togglePin(note.id); showToast(p ? 'Pinned' : 'Unpinned'); } },
      { label: 'Duplicate current note', hint: '', action: async () => { const dup = await duplicateNote(note.id); bus.emit('note:open', dup.id); showToast('Duplicated'); } },
      { label: 'Share current note', hint: '', action: () => document.getElementById('btn-share-note')?.click() },
    );
  }
  return cmds;
}
