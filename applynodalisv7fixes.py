...
def check_baseline(content):
    """Verify this is a v6-patched file - check for evidence of v6 changes"""
    # Check for multiple indicators of v6 patches to be flexible with different file states
    indicators = [
        ('v6 marker in comments', ['v6 patches applied', 'Nodalis v6']),
        ('Guest mode functions', ['function fetchNote', 'function guestError', 'function notFound']),
        ('Guest mode routing', ['#/share/', 'location.hash.startsWith']),
        ('Guest view', ['function renderGuestView', 'renderGuestView(gistId)']),
    ]

    matched = 0
    for category, strings in indicators:
        if any(s in content for s in strings):
            matched += 1

    # Need at least 2 categories to confirm v6 patches (flexible but safe)
    if matched >= 2:
        return True

    # Fallback: if file has BOTH fetchNote AND guestError, it's definitely v6-patched
    if 'function fetchNote' in content and 'function guestError' in content:
        return True
